...
### How many Orders, Families and Genera total
...
The below table gives the results of how many classes, orders, families and genera are in the dataset (excluding the 7 genera in the previous section that NCBI doesn't have complete taxonomy data on).

`r tax_summary_ncbi`

### Genera per Family, Families per Order, Orders per Class
...
tax_bins <- function(x,y,z){
  data <- data.frame(z %>% group_by_(y) %>% summarize_(n_uniq=interp(~n_distinct(v), v = as.name(x))))
  print(data)
  colnames(data) <- c(as.character(y), paste("n_", as.name(x), "_dataset", sep = ""))
  print(colnames(data))
  print(data)
...
```{r}
OperC <- data.frame(tax_ncbi %>% group_by(class) %>% summarize(n_distinct(order)))
colnames(OperC) <- c("class", "n_orders_dataset")

FperC <- data.frame(tax_ncbi %>% group_by(class) %>% summarize(n_distinct(family)))
colnames(FperC) <- c("class", "n_families_dataset")

GperC <- data.frame(tax_ncbi %>% group_by(class) %>% summarize(n_distinct(genus)))
colnames(GperC) <- c("class", "n_genera_dataset")
```


Next, Families and Genera per Order
```{r}
FperO <- data.frame(tax_ncbi %>% group_by(class) %>% summarize(n_distinct(order)))
colnames(FperO) <- c("class", "n_orders_dataset")

GperO <- data.frame(tax_ncbi %>% group_by(class) %>% summarize(n_distinct(family)))
colnames(GperO) <- c("class", "n_families_dataset")
```


Finally, Genera per Family
```{r}
GperF <- data.frame(tax_ncbi %>% group_by(family) %>% summarize(n_distinct(genus)))
colnames(GperF) <- c("family", "n_genera_dataset")
```
